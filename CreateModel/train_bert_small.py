#!/usr/bin/env python3
"""
BERT-small Model Eğitimi
Tarımsal classification için optimize edilmiş küçük BERT modeli
"""
...
class BERTSmallTrainer:
    """BERT-small model eğitim sınıfı"""
    
    def __init__(self, 
                 data_dir: str = "../Data",
                 output_dir: str = "bert_small_agricultural",
                 max_length: int = 512):
    ...
    def create_bert_small_model(self):
        """BERT-small modeli oluştur"""
        console.print("🤖 BERT-small modeli oluşturuluyor...", style="bold cyan")
        
        # BERT-small configuration
        config = BertConfig(
            vocab_size=30522,  # BERT base vocab size
            hidden_size=384,   # Küçültülmüş hidden size
            num_hidden_layers=6,  # Daha az layer
            num_attention_heads=6,  # Daha az attention head
    ...
    def setup_trainer(self, train_dataset, val_dataset):
        ...
        training_args = TrainingArguments(
            ...
            eval_strategy="epoch",
            eval_steps=100,
            save_strategy="no",  # DTensor problemini önlemek için save'i kapattım
            save_steps=100,
            save_total_limit=2,
    ...
    def train_model(self):
        """Model eğitimi"""
        console.print("🚀 BERT-small eğitimi başlıyor...", style="bold green")
    ...
    def evaluate_model(self, test_dataset):
        ...
        # Results table
        table = Table(title="BERT-small Test Sonuçları")
    ...
    def create_visualizations(self):
        ...
        fig.suptitle('BERT-small Training Analysis', fontsize=16, fontweight='bold')
        ...
        axes[1, 0].text(0.1, 0.8, 'BERT-small Architecture', fontsize=14, fontweight='bold')
        axes[1, 0].text(0.1, 0.6, f'Hidden Size: 384', fontsize=12)
        axes[1, 0].text(0.1, 0.5, f'Layers: 6', fontsize=12)
        axes[1, 0].text(0.1, 0.4, f'Attention Heads: 6', fontsize=12)
        axes[1, 0].text(0.1, 0.3, f'Parameters: ~14M', fontsize=12)
    ...
    def save_training_metrics_table(self):
        ...
        if self.training_history['train_loss']:
            metrics_table = Table(title="BERT-small Eğitim Metrikleri")
            ...
            ) if self.training_history['eval_loss'] else len(self.training_history['train_loss'])
            ...
            metrics_df = pd.DataFrame({
                ...
                'eval_loss': self.training_history['eval_loss'][:len(self.training_history['train_loss'])] if self.training_history['eval_loss'] else [],
                'eval_accuracy': self.training_history['eval_accuracy'][:len(self.training_history['train_loss'])] if self.training_history['eval_accuracy'] else [],
                'eval_f1': self.training_history['eval_f1'][:len(self.training_history['train_loss'])] if self.training_history['eval_f1'] else []
            })
...
def main():
    ...
    console.print("🤖 BERT-small Tarımsal Model Eğitimi", style="bold green")
    console.print("=" * 60)
    
    trainer = BERTSmallTrainer()
    ...
    try:
        ...
        # 2. Model oluşturma
        trainer.create_bert_small_model()
        ...
        # 8. Eğitim metrikleri kaydetme
        trainer.save_training_metrics_table()
        
        # Final özet
        console.print("\n✅ BERT-small eğitimi başarıyla tamamlandı!", style="bold green")
